...
class CustomWebhookSender:
    ...
    def _build_custom_webhook_template_payload(self, content: str) -> Optional[dict]:
        ...
        try:
            ...
            logger.error(
                "CUSTOM_WEBHOOK_BODY_TEMPLATE 不是有效 JSON，已回退爲默認 Webhook payload: %s",
                exc,
            )
            return None
        if not isinstance(payload, dict):
            logger.error(
                "CUSTOM_WEBHOOK_BODY_TEMPLATE 必須渲染爲 JSON object，已回退爲默認 Webhook payload"
            )
            return None
        return payload
