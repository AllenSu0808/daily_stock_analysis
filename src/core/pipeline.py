...
class StockAnalysisPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            max_workers: 最大並發線程數（可選，默認從配置讀取）
        """
        ...
        logger.info("已啓用技術分析引擎（均線/趨勢/量價指標）")
        # 打印實時行情/籌碼配置狀態
        if self.config.enable_realtime_quote:
            logger.info(f"實時行情已啓用 (優先級: {self.config.realtime_source_priority})")
        else:
            logger.info("實時行情已禁用，將使用歷史收盤價")
        if self.config.enable_chip_distribution:
            logger.info("籌碼分布分析已啓用")
        else:
            logger.info("籌碼分布分析已禁用")
        if self.search_service is None:
            logger.warning("搜索服務未啓用（初始化失敗或依賴缺失）")
        elif self.search_service.is_available:
            logger.info("搜索服務已啓用")
        else:
            logger.warning("搜索服務未啓用（未配置搜索能力）")
    ...
    def analyze_stock(
        ...
        """
        分析單只股票（增強版：含量比、換手率、籌碼分析、多維度情報）
        
        流程：
        1. 獲取實時行情（量比、換手率）- 通過 DataFetcherManager 自動故障切換
        2. 獲取籌碼分布 - 通過 DataFetcherManager 帶熔斷保護
        3. 進行趨勢分析（基於交易理念）
        ...
        try:
            ...
            try:
                if self.config.enable_realtime_quote:
                    ...
                    if realtime_quote:
                        ...
                        logger.warning(f"{stock_name}({code}) 所有實時行情數據源均不可用，已降級爲歷史收盤價繼續分析")
                else:
                    logger.info(f"{stock_name}({code}) 實時行情已禁用，使用歷史收盤價繼續分析")
            except Exception as e:
                logger.warning(f"{stock_name}({code}) 實時行情鏈路異常，已降級爲歷史收盤價繼續分析: {e}")

            # 如果還是沒有名稱，使用代碼作爲名稱
            if not stock_name:
                stock_name = f'股票{code}'

            # Step 2: 獲取籌碼分布 - 使用統一入口，帶熔斷保護
            chip_data = None
            try:
                ...
                if chip_data:
                    ...
                    logger.debug(f"{stock_name}({code}) 籌碼分布獲取失敗或已禁用")
            except Exception as e:
                logger.warning(f"{stock_name}({code}) 獲取籌碼分布失敗: {e}")
            ...
            if use_agent:
                logger.info(f"{stock_name}({code}) 啓用 Agent 模式進行分析")
        ...
        *,
        ...
        try:
            ...
            if report_language == "en":
                ...
                message = f"請分析股票 {code} ({stock_name})，並生成決策儀錶盤報告。"
            llm_started_at = time.monotonic()
            try:
                record_llm_run_started(
    ...
    def _agent_result_to_analysis_result(
        ...
        """
        將 AgentResult 轉換爲 AnalysisResult。
        """
        report_language = normalize_report_language(getattr(self.config, "report_language", "zh"))
        dash = None
        result = AnalysisResult(
            ...
            operation_advice="Watch" if report_language == "en" else "觀望",
        ...
        if agent_result.success and agent_result.dashboard:
            ...
            if isinstance(raw_advice, dict):
                ...
                result.operation_advice = str(raw_advice) if raw_advice else ("Watch" if report_language == "en" else "觀望")
            else:
            ...
            if not result.error_message:
                result.error_message = "Agent failed to generate a valid decision dashboard" if report_language == "en" else "Agent 未能生成有效的決策儀錶盤"
    ...
    def _apply_trend_fallback(
        ...
        if trend_result is None:
            ...
            result.operation_advice = "Watch" if report_language == "en" else "觀望"
            return
    ...
    def _resolve_query_source(self, query_source: Optional[str] = None) -> str:
        ...
        2. 存在 source_message 時推斷爲 "bot"：當前約定爲機器人會話上下文
        3. 存在 query_id 時推斷爲 "web"：Web 觸發的請求會帶上 query_id
    ...
    def process_single_stock(
        ...
        Args:
            ...
            single_stock_notify: 是否啓用單股推送模式（每分析完一隻立即推送）
    ...
    def run(
        ...
        Args:
            ...
            current_time: 本輪運行凍結的參考時間；爲空時在 run 內生成
        ...
        if len(stock_codes) >= 5:
            ...
            if prefetch_count > 0:
                logger.info(f"已啓用批量預取架構：一次拉取全市場數據，{len(stock_codes)} 只股票共享緩存")
        ...
        if single_stock_notify:
            logger.info(
                "已啓用單股推送模式：分析仍並發執行，通知改爲在結果收集側串行發送（報告類型: %s）",
                report_type_str,
            )
            ...
            for idx, future in enumerate(as_completed(future_to_code)):
                ...
                try:
                    ...
                    if idx < len(stock_codes) - 1 and analysis_delay > 0:
                        ...
                        # 因此它對降低並發請求峯值的效果有限；真正的峯值主要由 max_workers 決定。
                        # 該行爲目前保留（按需求不改邏輯）。
        ...
        # dry-run 模式下，數據獲取成功即視爲成功
        if dry_run:
            # 檢查哪些股票的最新可復用交易日數據已存在
            success_count = sum(
                1
                for code in stock_codes
    ...
    def _save_local_report(
        ...
        try:
            ...
            logger.info(f"決策儀錶盤日報已保存: {filepath}")
        except Exception as e:
        ...
        try:
            ...
            if self.notifier.is_available():
                ...
                if not should_broadcast_static:
                    ...
                    if send_context:
                        logger.info("決策儀錶盤推送成功")
                    else:
                        logger.warning("決策儀錶盤推送失敗")
                    logger.info("交互式消息上下文回復模式：已跳過靜態通知渠道")
                    return
                ...
                if non_wechat_channels_needing_image:
                    ...
                    if image_bytes:
                        logger.info(
                            "Markdown 已轉換爲圖片，將向 %s 發送圖片",
                            [ch.value for ch in non_wechat_channels_needing_image],
                        )
                    else:
                        logger.warning(
                            "Markdown 轉圖片失敗，將回退爲文本發送。請檢查 MARKDOWN_TO_IMAGE_CHANNELS 配置並安裝 %s",
                            _get_md2img_hint(),
                        )
                ...
                if NotificationChannel.WECHAT in channels:
                    def _send_wechat_report() -> bool:
                        ...
                        logger.info(f"企業微信儀錶盤長度: {len(dashboard_content)} 字符")
                        ...
                        if NotificationChannel.WECHAT in channels_needing_image:
                            ...
                            if wechat_image_bytes is None:
                                logger.warning(
                                    "企業微信 Markdown 轉圖片失敗，將回退爲文本發送。請檢查 MARKDOWN_TO_IMAGE_CHANNELS 配置並安裝 %s",
                                    _get_md2img_hint(),
                                )
