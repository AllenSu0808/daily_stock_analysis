...
class NotificationService(
    ...
    def __init__(self, source_message: Optional[BotMessage] = None):
        ...
        # 僅分析結果摘要（Issue #262）：true 時只推送匯總，不含個股詳情
    ...
    def _extract_telegram_context_chat_id(self) -> Optional[str]:
        """從來源消息中提取 Telegram 上下文 chat_id（用於異步回復）。"""
    ...
    def generate_dashboard_report(
        ...
        # 逐個股票的決策儀錶盤（Issue #262: summary_only 時跳過詳情）
    ...
    _SOURCE_DISPLAY_NAMES = {
        "tencent": {"zh": "騰訊財經", "en": "Tencent Finance", "zh-tw": "騰訊財經"},
        "akshare_em": {"zh": "東方財富", "en": "Eastmoney", "zh-tw": "東方財富"},
        "akshare_sina": {"zh": "新浪財經", "en": "Sina Finance", "zh-tw": "新浪財經"},
        "akshare_qq": {"zh": "騰訊財經", "en": "Tencent Finance", "zh-tw": "騰訊財經"},
        "efinance": {"zh": "東方財富(efinance)", "en": "Eastmoney (efinance)", "zh-tw": "東方財富(efinance)"},
        "tushare": {"zh": "Tushare Pro", "en": "Tushare Pro", "zh-tw": "Tushare Pro"},
        "sina": {"zh": "新浪財經", "en": "Sina Finance", "zh-tw": "新浪財經"},
        "stooq": {"zh": "Stooq", "en": "Stooq", "zh-tw": "Stooq"},
        "longbridge": {"zh": "長橋", "en": "Longbridge", "zh-tw": "長橋"},
        "fallback": {"zh": "降級兜底", "en": "Fallback", "zh-tw": "降級兜底"},
    }
    ...
    def _should_use_image_for_channel(
        ...
        if channel == NotificationChannel.WECHAT and len(image_bytes) > WECHAT_IMAGE_MAX_BYTES:
            logger.warning(
                "企業微信圖片超限 (%d bytes)，回退爲 Markdown 文本發送",
                len(image_bytes),
            )
            return False
        return True
    ...
    def send_with_results(
        ...
        Args:
            ...
            route_type: 通知路由類型；None 保持舊行爲，report/alert/system_error 按配置過濾靜態渠道
        ...
        if channels_needing_image:
            ...
            if image_bytes:
                logger.info("Markdown 已轉換爲圖片，將向 %s 發送圖片",
