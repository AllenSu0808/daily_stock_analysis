    ...
    if left and not right:
        target.append(
            _issue(
                ...
                f"{channel_name} 已配置 {left_key}，但缺少 {right_key}，該渠道不會啓用。",
                key=right_key,
            )
        )
    if right and not left:
        target.append(
            _issue(
                severity,
                "partial_channel_config",
                f"{channel_name} 已配置 {right_key}，但缺少 {left_key}，該渠道不會啓用。",
                key=left_key,
            )
        )
    ...
    if (_has(config, "feishu_webhook_secret") or _has(config, "feishu_webhook_keyword")) and not _has(config, "feishu_webhook_url"):
        warnings.append(
            _issue(
                ...
                "已配置飛書 Webhook 高級安全項，但缺少 FEISHU_WEBHOOK_URL，飛書 Webhook 渠道不會啓用。",
    ...
    if _has(config, "pushplus_topic") and not _has(config, "pushplus_token"):
        warnings.append(
            _issue(
                "warning",
                "advanced_without_minimal",
                "已配置 PUSHPLUS_TOPIC，但缺少 PUSHPLUS_TOKEN，PushPlus 渠道不會啓用。",
                key="PUSHPLUS_TOKEN",
            )
        )
    if _has(config, "ntfy_token") and not _has(config, "ntfy_url"):
        warnings.append(
            _issue(
                "warning",
                "advanced_without_minimal",
                "已配置 NTFY_TOKEN，但缺少 NTFY_URL，ntfy 渠道不會啓用。",
                key="NTFY_URL",
            )
        )
    if (
        ...
        warnings.append(
            _issue(
                "warning",
                "advanced_without_minimal",
                "已配置自定義 Webhook 高級項，但缺少 CUSTOM_WEBHOOK_URLS，自定義 Webhook 渠道不會啓用。",
    ...
    if _has(config, "astrbot_token") and not _has(config, "astrbot_url"):
        warnings.append(
            _issue(
                "warning",
                "advanced_without_minimal",
                "已配置 ASTRBOT_TOKEN，但缺少 ASTRBOT_URL，AstrBot 渠道不會啓用。",
                key="ASTRBOT_URL",
            )
        )
        ...
        if disabled_channels:
            warnings.append(
                _issue(
                    ...
                    (
                        f"{route_config['env_key']} 路由 {route_type} 指向未啓用渠道: "
