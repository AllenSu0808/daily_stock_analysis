...
async def auth_update_settings(request: Request, body: AuthSettingsRequest):
    ...
    if target_enabled:
        if password or confirm:
            if stored_password_exists:
                return JSONResponse(
                    ...
                    content={
                        ...
                        "message": "已存在管理員密碼，請啓用認證後通過修改密碼功能更新",
                    },
                )
            if not password:
                return JSONResponse(
                    status_code=400,
                    content={"error": "password_required", "message": "請輸入要設置的管理員密碼"},
                )
            if password != confirm:
                return JSONResponse(
                    status_code=400,
            ...
            return JSONResponse(
                status_code=400,
                content={"error": "password_required", "message": "開啓密碼登錄前請先設置密碼"},
            )
        else:
            ...
            if not is_valid_session:
                if not current_password:
                    return JSONResponse(
                        status_code=400,
                        content={"error": "current_required", "message": "重新開啓認證前請輸入當前密碼"},
                    )
