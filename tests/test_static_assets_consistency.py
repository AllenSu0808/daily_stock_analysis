...
def test_stock_index_route_serves_newer_remote_cache(tmp_path: Path) -> None:
    ...
    _write_stock_index(static_dir / "stocks.index.json", "內置靜態")
    _write_stock_index(cache_path, "遠程緩存", size=100)
    _write_stock_index(bundled_path, "源碼內置")
...
def test_stock_index_route_prefers_newer_static_index_over_older_remote_cache(tmp_path: Path) -> None:
    ...
    cache_path = tmp_path / "cache" / "stocks.index.json"
    bundled_path = tmp_path / "bundled" / "stocks.index.json"
    _write_stock_index(static_dir / "stocks.index.json", "新內置靜態")
    _write_stock_index(cache_path, "舊遠程緩存", size=100)
    _write_stock_index(bundled_path, "源碼內置")
    ...
    assert response.status_code == 200
    assert response.json()[0][2] == "新內置靜態"
